    ...
    public class DynamicAtlas
    {
        ...
        public void GetTeture(string name, OnCallBackTexRect callback)
        {
            ...
            else
            {
                ...
                GameEntry.Resource.LoadAsset(name, new LoadAssetCallbacks((string entityAssetName,
                 object entityAsset, float duration, object userData) =>
                {
                    Texture2D texture = (Texture2D)entityAsset;
                    if (texture == null)
                    {
                        Debug.LogError("Failed To load Texture:" + entityAssetName);
                        callback(null, new Rect(0, 0, 0, 0));
                        return;
                    }
                    Log.Info($"DynamicAtlas - LoadAsset:{entityAssetName}");
                    GetTextureData data = DynamicAtlasMgr.S.AllocateGetTextureData();
                    data.name = texture.name;
                    data.callback = callback;
                    m_GetTextureTaskList.Add(data);

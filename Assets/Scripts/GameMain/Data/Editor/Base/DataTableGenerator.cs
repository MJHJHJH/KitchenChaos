...
namespace GameFramework.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        {
            return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(DataTableConst.DataTablePath, dataTableName + ".txt")), Encoding.GetEncoding("GB2312"), 1, 2, null, 3, 4, 1);
        }
        ...
        {
            string inputFilePath = Utility.Path.GetRegularPath(Path.Combine(DataTableConst.DataTablePath, dataTableName + ".txt"));
            string outFilePath = Utility.Path.GetRegularPath(Path.Combine(DataTableConst.DataTableByte_Path, dataTableName + ".bytes"));
            if (!dataTableProcessor.GenerateDataFile(inputFilePath, outFilePath) && File.Exists(inputFilePath))
            {
        ...
        {
            dataTableProcessor.SetCodeTemplate(DataTableConst.CSharpCodeTemplateFileName, Encoding.UTF8);
            dataTableProcessor.SetCodeGenerator(DataTableCodeGenerator);

            string csharpCodeFileName = Utility.Path.GetRegularPath(Path.Combine(DataTableConst.CSharpCodePath, "DR" + dataTableName + ".cs"));

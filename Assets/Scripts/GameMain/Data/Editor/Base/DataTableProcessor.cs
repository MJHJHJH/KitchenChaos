...
namespace GameFramework.Editor.DataTableTools
{
    public sealed partial class DataTableProcessor
    {
        ...
        public bool GenerateDataFile(string inputFilePath, string outputFilePath)
        {
            if (string.IsNullOrEmpty(inputFilePath))
            {
                throw new GameFrameworkException("input file name is invalid.");
            }
            ...
            try
            {
                using (FileStream fileStream = new FileStream(outputFilePath, FileMode.Create, FileAccess.Write))
                {
                    using (BinaryWriter binaryWriter = new BinaryWriter(fileStream, Encoding.UTF8))
                    {
                        for (int rawRow = ContentStartRow; rawRow < RawRowCount; rawRow++)
                        {
                            ...
                            byte[] bytes = GetRowBytes(inputFilePath, rawRow);
                ...
                Debug.Log(Utility.Text.Format("Parse data table '{0}' success.", outputFilePath));
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogError(Utility.Text.Format("Parse data table '{0}' failure, exception is '{1}'.", outputFilePath, exception.ToString()));
                return false;
            }
        }
